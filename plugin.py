...
"""
"""
<plugin key="OM310" name="OM-310 Power Limiter (Novatek-Elektro)" version="0.0.1" author="tmrealive">
    <params>
...
class BasePlugin:
    ...
    def onStart(self):
        ...
        if 1 not in Devices:
            Domoticz.Device(Name="Current L1 (IF1)",Unit=1,TypeName="Custom",Used=1,Options={"Custom":"1;A"}).Create()
        if 2 not in Devices:
            Domoticz.Device(Name="Current L2 (IF2)",Unit=2,TypeName="Custom",Used=1,Options={"Custom":"1;A"}).Create()
        if 3 not in Devices:
            Domoticz.Device(Name="Current L3 (IF2)",Unit=3,TypeName="Custom",Used=1,Options={"Custom":"1;A"}).Create()
        if 4 not in Devices:
            Domoticz.Device(Name="Zero Sequence Current (IF0)",Unit=4,TypeName="Custom",Used=0,Options={"Custom":"1;A"}).Create()
        if 5 not in Devices:
            Domoticz.Device(Name="Average Current L1 (IS1)",Unit=5,TypeName="Custom",Used=0,Options={"Custom":"1;A"}).Create()
        if 6 not in Devices:
            Domoticz.Device(Name="Average Current L2 (IS2)",Unit=6,TypeName="Custom",Used=0,Options={"Custom":"1;A"}).Create()
        if 7 not in Devices:
            Domoticz.Device(Name="Average Current L3 (IS3)",Unit=7,TypeName="Custom",Used=0,Options={"Custom":"1;A"}).Create()
        if 8 not in Devices:
            Domoticz.Device(Name="Maximum Current L1 (IN1)",Unit=8,TypeName="Custom",Used=0,Options={"Custom":"1;A"}).Create()
        if 9 not in Devices:
            Domoticz.Device(Name="Maximum Current L2 (IN2)",Unit=9,TypeName="Custom",Used=0,Options={"Custom":"1;A"}).Create()
        if 10 not in Devices:
            Domoticz.Device(Name="Maximum Current L3 (IN3)",Unit=10,TypeName="Custom",Used=0,Options={"Custom":"1;A"}).Create()
        if 11 not in Devices:
            Domoticz.Device(Name="Negative Sequence Current (IOP)",Unit=11,TypeName="Custom",Used=0,Options={"Custom":"1;A"}).Create()

        if 12 not in Devices:
            Domoticz.Device(Name="Voltage L1 (UF1)",Unit=12,TypeName="Custom",Used=1,Options={"Custom":"1;V"}).Create()
        if 13 not in Devices:
            Domoticz.Device(Name="Voltage L2 (UF2)",Unit=13,TypeName="Custom",Used=1,Options={"Custom":"1;V"}).Create()
        if 14 not in Devices:
            Domoticz.Device(Name="Voltage L3 (UF3)",Unit=14,TypeName="Custom",Used=1,Options={"Custom":"1;V"}).Create()
        if 15 not in Devices:
            Domoticz.Device(Name="Line Voltage L12 (UL1)",Unit=15,TypeName="Custom",Used=0,Options={"Custom":"1;V"}).Create()
        if 16 not in Devices:
            Domoticz.Device(Name="Line Voltage L23 (UL2)",Unit=16,TypeName="Custom",Used=0,Options={"Custom":"1;V"}).Create()
        if 17 not in Devices:
            Domoticz.Device(Name="Line Voltage L31 (UL3)",Unit=17,TypeName="Custom",Used=0,Options={"Custom":"1;V"}).Create()
        if 18 not in Devices:
            Domoticz.Device(Name="Positive Sequence Voltage (UPP)",Unit=18,TypeName="Custom",Used=0,Options={"Custom":"1;V"}).Create()
        if 19 not in Devices:
            Domoticz.Device(Name="Negative Sequence Voltage (UOP)",Unit=19,TypeName="Custom",Used=0,Options={"Custom":"1;V"}).Create()
        if 20 not in Devices:
            Domoticz.Device(Name="Zero Sequence Voltage (UNP)",Unit=20,TypeName="Custom",Used=0,Options={"Custom":"1;V"}).Create()

        if 21 not in Devices:
            Domoticz.Device(Name="Apparent Power (POT)",Unit=21,TypeName="Custom",Used=0,Options={"Custom":"1;kVA"}).Create()
        if 22 not in Devices:
            Domoticz.Device(Name="Active Power (POA)",Unit=22,TypeName="Custom",Used=1,Options={"Custom":"1;kW"}).Create()
        if 23 not in Devices:
            Domoticz.Device(Name="Reactive Power (POJ)",Unit=23,TypeName="Custom",Used=0,Options={"Custom":"1;kVAR"}).Create()
        if 24 not in Devices:
            Domoticz.Device(Name="Active Power L1 (PAA)",Unit=24,TypeName="Custom",Used=1,Options = {"Custom":"1;kW"}).Create()
        if 25 not in Devices:
            Domoticz.Device(Name="Active Power L2 (PAB)",Unit=25,TypeName="Custom",Used=1,Options = {"Custom":"1;kW"}).Create()
        if 26 not in Devices:
            Domoticz.Device(Name="Active Power L3 (PAC)",Unit=26,TypeName="Custom",Used=1,Options = {"Custom":"1;kW"}).Create()

        if 27 not in Devices:
            Domoticz.Device(Name="Cosine L1 (PCA)",Unit=27,TypeName="Custom",Used=0,Options={"Custom":"1;°"}).Create()
        if 28 not in Devices:
            Domoticz.Device(Name="Cosine L2 (PCB)",Unit=28,TypeName="Custom",Used=0,Options={"Custom":"1;°"}).Create()
        if 29 not in Devices:
            Domoticz.Device(Name="Cosine L3 (PCC)",Unit=29,TypeName="Custom",Used=0,Options={"Custom":"1;°"}).Create()

        if 30 not in Devices:
            Domoticz.Device(Name="Overload Timer Off (T0P)",Unit=30,TypeName="Custom",Used=0,Options={"Custom":"1;s"}).Create()
        if 31 not in Devices:
            Domoticz.Device(Name="Automatic Timer On (TAP)",Unit=31,TypeName="Custom",Used=0,Options={"Custom":"1;s"}).Create()
        if 32 not in Devices:
            Domoticz.Device(Name="Overload Timer On (TTP)",Unit=32,TypeName="Custom",Used=0,Options={"Custom":"1;s"}).Create()

        if 33 not in Devices:
            Domoticz.Device(Name="Frequency (FFF)",Unit=33,TypeName="Custom",Used=0,Options={"Custom":"1;Hz"}).Create()

        if 34 not in Devices:
            Domoticz.Device(Name="Energy L1",Unit=34,TypeName="Custom",Used=0,Options={"Custom":"1;kVA/h"}).Create()
        if 35 not in Devices:
            Domoticz.Device(Name="Energy L2",Unit=35,TypeName="Custom",Used=0,Options={"Custom":"1;kVA/h"}).Create()
        if 36 not in Devices:
            Domoticz.Device(Name="Energy L3",Unit=36,TypeName="Custom",Used=0,Options={"Custom":"1;kVA/h"}).Create()
        if 37 not in Devices:
            Domoticz.Device(Name="Energy",Unit=37,TypeName="Custom",Used=0,Options={"Custom":"1;kVA/h"}).Create()
    ...
    def onHeartbeat(self):
        ...
        itemEL = itemEL1 + itemEL2 + itemEL3 + float(Parameters["Mode3"])
        ...
        Devices[37].Update(0,str(itemEL))
